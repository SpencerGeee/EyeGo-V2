 ...
 *
 * WHY NOT OSRM DIRECTLY: the active-trip and tracking screens used to call
 * `router.project-osrm.org` themselves. That public demo server returns
 * FREE-FLOW duration — the time the drive would take on an empty road — which
 * is why the driver app quoted an 8.3 km trip as ~12 minutes (≈41 km/h through
 * Accra traffic). It is also unauthenticated, rate-limited and unguaranteed.
 *
 * The proxy uses Mapbox's `driving-traffic` profile and falls back internally,
 * so callers here get one shape and never a blank ETA.
 *
 * Mirrors apps/rider/utils/routing.ts — kept as two copies rather than a shared
 * package because the apps do not currently share a utils package and adding one
 * for two files would be a heavier change than the duplication.
 */
