...
| E5 | high | **`next build` for the admin console failed outright.** It died on `Export encountered an error on /_error: /404` with a null `useRef` — the signature of two React copies. The cause: `node_modules/react-dom` at the repo root was a **corrupted partial install** containing a single `LICENSE` file and no `index.js`, so `react` resolved to the root while `react-dom` resolved to a nested copy under `apps/admin`. Two module instances, so react-dom's dispatcher was not the react whose hooks were running. Repaired by making both resolve to one copy at the root; the build now completes and renders every route. This was a local `node_modules` artifact rather than a repo defect — the admin Dockerfile does a clean `npm ci` and would not have inherited it — but it meant the console could not be built at all on this machine, and the done bar requires that build. |
